namespace MinimalApi.Api.Beers.List.ByBrewery;

public static class Endpoint
{
    public static RouteHandlerBuilder MapBeersListByBrewery(this RouteGroupBuilder group)
        => group.MapGet("/breweries/{id}/beers", Delegate)
                .Produces<BeerList>(StatusCodes.Status200OK)
                .Produces(StatusCodes.Status204NoContent)
                .Produces(StatusCodes.Status404NotFound)
                .WithName("GetBeersByBrewery");

    private static async Task<IResult> Delegate(BeerDbContext db, HashedId id, int page = 1, int pageSize = 10, CancellationToken cancellationToken = default)
    {
