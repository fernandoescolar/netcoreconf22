namespace Example.Library.Endpoints;
...
public abstract record Endpoint<TInput, TOutput>(string[] Verbs, string Path) : Endpoint(Verbs, Path)
{
    ...
    public Task<TOutput> HandleAsync([AsParameters]TInput request, CancellationToken cancellationToken) => OnHandleAsync(request, cancellationToken);
